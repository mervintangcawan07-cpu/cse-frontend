  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-2 py-3 sm:px-4 sm:py-6 md:px-6 text-slate-100">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl sm:rounded-3xl shadow-xl overflow-hidden">
        ...
        {/* ATTEMPTS LIST (SHOWING ONLY TOP 3) INSIDE UNIFIED FRAME */}
        <div className="p-3.5 sm:p-6 md:p-8 space-y-4">
          {attempts.length === 0 ? (
            <div className="bg-slate-950 border border-slate-800/80 rounded-2xl sm:rounded-3xl p-12 text-center space-y-4">
            ...
            <div className="space-y-3">
              ...
              {recent3Attempts.map((item, index) => {
                ...
                return (
                  <div
                    key={item.id}
                    className="bg-slate-950/80 border border-slate-800 rounded-2xl p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:border-slate-700 transition"
                  >
